...
void Unit::DealMeleeDamage(CalcDamageInfo* damageInfo, bool durabilityLoss)
{
    ...
    if (damageInfo->Damages[0].Damage + damageInfo->Damages[1].Damage)
    {
        ...
        for (AuraEffect const* aurEff : vDamageShieldsCopy)
        {
            ...
            {
                damage = caster->SpellDamageBonusDone(this, spellInfo, spellInfo->GetSchoolMask(), damage, SPELL_DIRECT_DAMAGE, aurEff->GetSpellEffectInfo(), { });
...
uint32 Unit::SpellDamageBonusDone(Unit* victim, SpellInfo const* spellProto, SpellSchoolMask schoolMask, uint32 pdamage, DamageEffectType damagetype, SpellEffectInfo const& spellEffectInfo, Optional<float> const& donePctTotal, uint32 stack /*= 1*/) const
{
    ...
    if (GetTypeId() == TYPEID_UNIT && IsTotem())
        if (Unit* owner = GetOwner())
            return owner->SpellDamageBonusDone(victim, spellProto, schoolMask, pdamage, damagetype, spellEffectInfo, donePctTotal, stack);
