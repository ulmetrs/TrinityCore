...
void WorldSession::SendAuctionHello(ObjectGuid guid, Unit const* unit)
{
    ...
    AuctionHouseEntry const* ahEntry = AuctionHouseMgr::GetAuctionHouseByFactionTemplateId(unit->GetFaction());
    if (!ahEntry)
        return;
...
void WorldSession::HandleAuctionRemoveItem(WorldPacket& recvData)
{
    ...
    if (auction && auction->owner == player->GetGUID().GetCounter())
    {
        ...
        if (pItem)
        {
            ...
            MailDraft(sAuctionMgr->BuildAuctionMailSubject(auction, AUCTION_CANCELED), "")
                .AddItem(pItem)
