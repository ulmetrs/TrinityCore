...
void AuraEffect::HandleSpiritOfRedemption(AuraApplication const* aurApp, uint8 mode, bool apply) const
{
    ...
    // Since these immunities must be missing from the spell itself apply them manually
    target->ApplySpellImmune(0, IMMUNITY_SCHOOL, SPELL_SCHOOL_MASK_ALL, apply);
    target->ApplySpellImmune(0, IMMUNITY_MECHANIC, IMMUNE_TO_MOVEMENT_IMPAIRMENT_AND_LOSS_CONTROL_MASK, apply);
...
void AuraEffect::HandleAuraModShapeshift(AuraApplication const* aurApp, uint8 mode, bool apply) const
{
    ...
    if (apply)
    {
        ...
        TC_LOG_DEBUG("charge", "AuraEffect::HandleAuraModShapeshift Setting ShapeshiftForm {} - {}", GetId(), GameTime::GetGameTimeMS());
