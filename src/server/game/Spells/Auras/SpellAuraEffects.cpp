        ...
        switch (GetAuraType())
        {
            ...
            case SPELL_AURA_PERIODIC_LEECH:
                if (GetBase()->GetType() == UNIT_AURA_TYPE)
                    amount = caster->SpellDamageBonusDone(GetBase()->GetUnitOwner(), GetSpellInfo(), GetSpellInfo()->GetSchoolMask(), amount, DOT, GetSpellEffectInfo(), GetBase()->GetDonePct());
                break;
...
void AuraEffect::HandlePeriodicDamageAurasTick(Unit* target, Unit* caster) const
{
    ...
    if (GetAuraType() == SPELL_AURA_PERIODIC_DAMAGE)
    {
        ...
        if (GetBase()->GetType() == DYNOBJ_AURA_TYPE)
            damage = caster->SpellDamageBonusDone(target, GetSpellInfo(), GetSpellInfo()->GetSchoolMask(), damage, DOT, GetSpellEffectInfo(), { }, GetBase()->GetStackAmount());
...
void AuraEffect::HandlePeriodicHealthLeechAuraTick(Unit* target, Unit* caster) const
{
    ...
    // dynobj auras must always have a caster
    if (GetBase()->GetType() == DYNOBJ_AURA_TYPE)
        damage = ASSERT_NOTNULL(caster)->SpellDamageBonusDone(target, GetSpellInfo(), GetSpellInfo()->GetSchoolMask(), damage, DOT, GetSpellEffectInfo(), { }, GetBase()->GetStackAmount());
    damage = target->SpellDamageBonusTaken(caster, GetSpellInfo(), GetSpellInfo()->GetSchoolMask(), damage, DOT);
